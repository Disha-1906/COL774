...
def train_naive_bayes(X_train, y_train, vocab, label_map):
    ...
    feature_log_complements = np.zeros((n_classes, len(vocab)))
...
def predict_naive_bayes(X_test, test_texts, vocab, class_probabilities, feature_probabilities, feature_log_complements, labels, y_train, label_map):
    ...
    for x, xx in zip(X_test, test_texts):
        ...
        for c in range(len(class_probabilities)):
            class_scores[c] += np.sum(x * feature_probabilities[c] + (1 - x) * feature_log_complements[c])
            if doc - set(vocab):  # Handle unseen words
...
def main():
    ...
    # Load stopwords from the file (handle any blank lines)
    ...
    # Train the model
    class_probabilities, feature_probabilities, feature_log_complements = train_naive_bayes(X_train, y_train, vocab, label_map)
    ...
    # Output the predictions to the specified output file (handle no trailing newline)
    with open(args.out, 'w') as f_out:
        f_out.write('\n'.join(y_pred))  # This ensures no extra newline at the end of the file
